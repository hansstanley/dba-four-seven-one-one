...
edges_0302 <- read.table("amazon0302.txt", header=F, skip=4, sep="\t")
edges_0601 <- read.table("amazon0601.txt", header=F, skip=4, sep="\t")
...
raw_split <- strsplit(raw_table$Raw, ":", fixed=T)
...
G_full = graph_from_data_frame(edges_0302, directed=F, nodes_df)

# prune nodes with degree less than `max_degree` so that
# graphs can be rendered in a reasonable amount of time
max_degree <- 50
G_degrees <- degree(G_full, mode = "all")
G_orphans <- which(G_degrees < max_degree)
...
plot(G, layout=mylayout, vertex.label=NA, vertex.size=2*sqrt(degree(G)),
     vertex.color = get.colors(c("white", "purple"), 2^degree(G)))
...
sg = induced_subgraph(G_full, in.max.comp)
# get the sub layout
sg_mylayout = mylayout[in.max.comp,]
plot(sg, layout=sg_mylayout)
...
sg.degree <- data.frame(deg=degree(sg))
sg.degree$Id <- row.names(sg.degree)
sg.degree <- sg.degree[order(sg.degree$deg, decreasing = T),]
sg.degree.100 <- sg.degree[1:100,]
sg.degree.100 <- merge(sg.degree.100, nodes_df, by="Id")
sg.degree.100 <- sg.degree.100[order(sg.degree.100$deg, decreasing = T),]
...
# appealing.
plot(sg, layout=mylayout, vertex.label=NA,
...
plot(sg, layout=sg_mylayout, vertex.label=NA, vertex.size=100*sqrt(sg_cl), 
     vertex.color = get.colors(c("white", "purple"), closeness(sg)^10))
